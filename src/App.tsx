...
import { View3D } from './elements/emotiondrawing/View3D';
import type { EmotionDrawingElement } from './elements/emotiondrawing/types';
import './App.css';
...
function shouldKeepOriginalStrokesForResult(result: { elements: Element[] }): boolean {
  return result.elements.some((el) => el.type === 'emotiondrawing');
}

function getPreservedStrokeElementsForResult(
  latestElements: Element[],
  result: { elements: Element[]; consumedStrokes: Stroke[] }
): Element[] {
  const keepOriginalStrokes = shouldKeepOriginalStrokesForResult(result);
  if (!keepOriginalStrokes) return latestElements;

  const preservedStrokeElement = createStrokeElement(result.consumedStrokes);
  return [...latestElements, preservedStrokeElement];
}
...
function App() {
  ...
  // Estado para la vista 3D del elemento EmotionDrawing
  const [view3dElement, setView3dElement] = useState<EmotionDrawingElement | null>(null);
  ...
  // Elemento EmotionDrawing seleccionado (si hay exactamente uno)
  const selectedEmotionElement = useMemo<EmotionDrawingElement | null>(() => {
    const ed = selectedElements.find((el) => el.type === 'emotiondrawing');
    return (ed as EmotionDrawingElement | undefined) ?? null;
  }, [selectedElements]);
  ...
  const processStrokes = useCallback(async (strokes: Stroke[]) => {
    ...
    debugLog.info('processStrokes called', {
      pendingCount: pendingStrokesRef.current.length,
      newStrokes: strokes.length,
    });

    let rectXRejection = 'not enough strokes';

    if (pendingStrokesRef.current.length >= 3) {
      debugLog.info('Attempting rectangle+X detection', {
        pendingCount: pendingStrokesRef.current.length,
      });
    ...
    if (pendingStrokesRef.current.length >= 1) {
      debugLog.info('Attempting element creation with disambiguation', {
        pendingStrokes: pendingStrokesRef.current.length,
      });

      // Usar todos los strokes pendientes para darle más contexto a emotiondrawing
      const recentStrokes = pendingStrokesRef.current;
        ...
        if (disambigResult.result && disambigResult.result.elements.length > 0) {
          ...
          if (disambigResult.needsDisambiguation && disambigResult.candidates) {
            debugLog.info('Cross-type disambiguation needed', {
              ...
              candidates: disambigResult.candidates.map(
                c => `${c.elementType}:${c.label}(${c.confidence.toFixed(2)})`
              ),
            });

            const intent = createDisambiguationIntent(
              disambigResult.candidates,
              result.consumedStrokes
            );
          ...
          const consumedIds = result.consumedElementIds
            ? new Set(result.consumedElementIds)
            : null;

          const keepOriginalStrokes = shouldKeepOriginalStrokesForResult(result);

          const baseElements = keepOriginalStrokes
            ? getPreservedStrokeElementsForResult(latestElements, result)
            : removeConsumedStrokeElements(latestElements, consumedSet);

          const surviving = baseElements.filter(
            (el) => !consumedIds || !consumedIds.has(el.id)
          );
      ...
      if (cluster && cluster.strokes.length >= 4) {
        try {
          ...
          if (result && result.elements.length > 0) {
            ...
            setCurrentNote({
              ...currentNoteRef.current,
              elements: [...baseElements, ...result.elements],
            });
    ...
    if (pendingStrokesRef.current.length >= 2) {
      debugLog.info('Attempting InkText clustering', {
        pendingStrokes: pendingStrokesRef.current.length,
      });
      ...
      if (cluster && cluster.strokes.length >= 2) {
        ...
        debugLog.warn('No valid cluster found for InkText', {
          pendingStrokes: pendingStrokesRef.current.length,
        });
      }
    }
    ...
    if (rectXRejection) {
      debugLog.warn('RectX detection failed', {
        reason: rectXRejection,
        pendingStrokes: pendingStrokesRef.current.length,
      });
    }

    const strokeElement = createStrokeElement(strokes);
    debugLog.info('Added stroke element', {
      id: strokeElement.id.slice(0, 8),
        ...
        if (plugin && plugin.createFromInk) {
          try {
            ...
            if (result && result.elements.length > 0) {
              ...
              const keepOriginalStrokes = result.elements.some(
                (el) => el.type === 'emotiondrawing'
              );

              setCurrentNote({
                ...currentNote,
                elements: keepOriginalStrokes
                  ? [...remainingElements, createStrokeElement(strokes), ...result.elements]
                  : [...remainingElements, ...result.elements],
              });
            } else {
  ...
  return (
    <div className="app">
      ...
      <header className="toolbar">
        ...
        {/* Botón 3D — visible solo cuando hay un EmotionDrawing seleccionado */}
        {selectedEmotionElement && (
          <>
            <div className="toolbar-divider" />
            <div className="toolbar-section tool-buttons">
              <button
                onClick={() => setView3dElement(selectedEmotionElement)}
                title="Ver dibujo en 3D"
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', fontSize: 12, fontFamily: 'sans-serif' }}
              >
